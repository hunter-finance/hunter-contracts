...
contract ExtinctionPool is Ownable {
    ...
    address constant burnAddress = 0x2BB1BDcf833Fd8659E22A575F3b1A7c5CEE6FaB9;

    struct UserInfo {
        uint256 amount; // How many HUNTER tokens the user has provided.
    ...
    uint256 public stakedHunters;
    PoolInfo public poolInfo;
    bool public emergencyActive;

    mapping(address => UserInfo) public userInfo; // Info of each user that stakes HUNTER tokens.

    event Rewarded(address indexed user, uint256 amount);
    event DepositAndBurn(address indexed user, uint256 amount);
    ...
    constructor(
        ...
        require(address(_HUNTER) != address(0), "_HUNTER address not set!");
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > poolInfo.lastRewardBlock && stakedHunters != 0) {
            ...
            accRewardPerShare = accRewardPerShare.add(
                tokenReward.mul(1e12).div(stakedHunters)
            );
        }
        return
    ...
    /**
     * @dev Deposit HUNTER tokens to Faucet for rewards allocation and/or withdraw outstanding rewards.
     * @param _amount Amount of HUNTER tokens to deposit.
     */
    function transact(uint256 _amount) public {
        require(!emergencyActive, "Active emergency.");
        ...
        if (user.amount > 0) {
            ...
            emit Rewarded(msg.sender, pending);
        }
        if (block.number < endBlock && _amount != 0) {
            HUNTER.safeTransferFrom(address(msg.sender), burnAddress, _amount);
            stakedHunters = stakedHunters.add(_amount);
            user.amount = user.amount.add(_amount);

            emit DepositAndBurn(msg.sender, _amount);
        }
